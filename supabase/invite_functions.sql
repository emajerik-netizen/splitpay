...
-- 5. Owner can invite a registered user by email and copy the trip to their account
--    - Caller must be the owner of the trip (found by invite code)
--    - Looks up target user by email and inserts the trip into their trip_states
CREATE OR REPLACE FUNCTION invite_user_by_email(p_invite_code TEXT, p_member_name TEXT, p_target_email TEXT)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  FROM user_profiles
  WHERE lower(user_email) = lower(trim(p_target_email))
  LIMIT 1;
